setwd("/home/anirjit/ANIRJIT/ROC2/human/lcr_num/roc2")
...
# Read the data
df2 <- read.delim("all.tsv", header = TRUE, sep = "\t")

# Convert LCR_num to a factor to ensure all values appear on the x-axis
df2$LCR_num <- factor(df2$LCR.num, levels = as.character(sort(unique(df2$LCR_num))))
...
ggplot(df2, aes(x = LCR.num, y = TPR, color = Tool, group = Tool)) +
  geom_line(size = 1) +
  geom_point(size = 2) + 

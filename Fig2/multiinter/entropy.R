...
for (file in file_list) {
  ...
  entropy_data[[gsub("_gf_entropy_purity.tsv", "", file)]] <- df$Entropy
}

# Open a PNG device to save the plot
jpeg("entropy_boxplot_sorted_baseR.jpg", width =1500, height = 1200, res = 250, quality = 50)

# Create a boxplot using base R
boxplot(entropy_data, 
        main = "Boxplot of Entropy Values across Groups of Tools", 
        xlab = "Number of tools", 
        ylab = "Shannon's Entropy",
        col = rainbow(length(entropy_data)),   # Assign different colors

...
p1 <- ggplot(df, aes(x = Ratio, y = TPR, group = Tool, color = Tool)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_manual(values = tool_colors) +
  labs(x = "LCR:Gene Entropy Ratio",
...
p2 <- ggplot(df, aes(x = Ratio, y = FPR, group = Tool, color = Tool)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_manual(values = tool_colors) +
  labs(x = "LCR:Gene Entropy Ratio",
...
jpeg("plots_entropy.jpeg", width = 3000, height = 2000, res = 250, quality = 75)

...
df$LCR_num <- factor(df$LCR.num, levels = as.character(sort(unique(df$LCR.num))))
...
p1 <- ggplot(df, aes(x = LCR.num, y = TPR, color = Tool, group = Tool)) +
  geom_line(size = 1) +
  geom_point(size = 2) + 
...
p2 <- ggplot(df, aes(x = LCR.num, y = FPR, color = Tool, group = Tool)) +
  geom_line(size = 1) +
  geom_point(size = 2) + 
  scale_color_manual(values = tool_colors) + 
  labs(x = "LCR Number",
...
jpeg("plots_lcr_num.jpeg", width = 3000, height = 2000, res = 250, quality = 75)
